...
| $0800  | $0800 | 32 tables x 64 bytes: 16 rows of (V, TSP, CMD, VAL) — voice level 00-7F, signed transpose, one command+value through the shared executor; $00 = no change per column (2026-07-11, was two cmd/val pairs) — run per tick by the instrument's TABLE field (record byte 12) |
