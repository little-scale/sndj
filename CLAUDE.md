...
```
A  arpeggio            H  hop                 R  retrig
B  wave-bank select    I  iteration play-mask S  sweep
C  chord/GRP override  K  kill note           T  tempo
D  delay note          L  slide/legato        V  vibrato
E  envelope select     M  master vol fade     W  wait
F  fine pitch          N  noise clock (glob)  
G  groove select       P  pan (signed L/R)
```

SNES-specific additions (letters chosen to avoid sibling collisions;
X/Y stay "chip-special" as in smsggdj-FM):

```
O xx  sample offset — start at BRR block xx
...
X xy  echo — x: EON on/off for this voice, y: echo return level nudge
Y 0x  FIR preset select (global, from the 8-curve bank)
Z xx  pitch-mod depth/enable on this voice (uses left neighbour)
```
...
- **Per-song echo config** in PROJECT (EDL, feedback, EVOL L/R, FIR preset);
  **per-voice sends** in INSTR/ECHO; **per-tick motion** via `X`/`Y`
...
- **M9 — Echo & FIR.** ECHO/FIR screens, safe-reconfig service, X/Y
