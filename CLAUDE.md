...
```
A  arpeggio            J  pass-transpose      R  retrig
B  wave-bank select    K  kill note           S  sweep
C  chord override      L  slide/legato        T  tempo
D  delay note          M  master volume       V  vibrato
...
SNES-specific additions (X/Y stay "chip-special" as in smsggdj-FM):
...
X xy  echo — EON on/off for this voice
Y 0x  FIR preset select (global; recalls into the song's taps)
Z xx  pitch-mod enable on this voice (uses left neighbour)
```

⚖ SETTLED (Seb, 2026-07-09): E (envelope select), O (sample offset)
and W are dropped — genmddj repurposed W to scale changes and none of
the three earn their letter here. The set above is complete: 23 live.
