...
**0.01** (v0.1 → v0.11 → v0.12 → v0.13 → v0.14 → …). The build stamps the version + git
...
1. **Bump the version** in `src/main.asm`:
   ...
   ```
   .DEFINE VERSION "0.14"      ; +0.01
   ```

   The `Makefile` derives every filename from it (`sndj-0.14-<hash>.sfc`
   dev copies, `make dist` → `sndj-0.14.sfc`), so nothing else needs
   editing.
...
3. **Commit _and push_** — both, before creating the release:
   ...
   ```
   git add -A && git commit -m "release: bump to v0.14 + changelog"
...
5. **Write release notes** (highlights + a link to `CHANGELOG.md`), then:
   ...
   ```
   gh release create v0.14 build/sndj-0.14.sfc \
     --title "sndj v0.14" --notes-file notes.md --target main
   ```
...
6. **Verify** the tag matches HEAD:
   ...
   ```
   git rev-parse HEAD
   git ls-remote --tags origin v0.14
   ```

   If they differ, you forgot step 3's push —
   `gh release delete v0.14 --cleanup-tag --yes`, push, re-create.

## Notes

- The **release asset** uses the clean name `sndj-0.14.sfc`; the hash
