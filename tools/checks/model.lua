...
emu.addEventCallback(function()
  ...
  if frames == 70 then
    ...
    check(wram(0x4800) == 0, "SONG V1 row0 = chain 00")
    check(wram(0x4880) == 1, "SONG V2 row0 = chain 01")
  elseif frames == 110 then
    check(wram(0x000C) == 1, "A+Right descended into PHRASE")
    check(wram(0x0018) == 1, "editing phrase 01")
    check(wram(0x4020) == 1, "chain 01 entry0 -> phrase 01")
    check(wram(0x4021) == 12, "chain 01 entry0 transpose +12")
  elseif frames == 192 then
    check(wram(0x000C) == 3, "A+Left climbed back to SONG")
    check(wram(0x4000) == 0, "chain 00 entry0 -> phrase 00")
    check(wram(0x4001) == 0, "chain 00 entry0 transpose 0")
    check(wram(0x2000) == 49, "phrase 00 row0 = C-4")
    check(wram(0x2040) == 49, "phrase 01 row0 = C-4")
